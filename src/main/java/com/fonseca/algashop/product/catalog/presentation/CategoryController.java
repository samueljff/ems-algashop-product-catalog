...
public class CategoryController {
    ...
    @GetMapping
    public ResponseEntity<PageModel<CategoryDetailOutput>> filter(CategoryFilter  categoryFilter, WebRequest webRequest) {
        ...
        OffsetDateTime lastModified = categoryQueryService.lasModified();
    ...
    public ResponseEntity<CategoryDetailOutput> findById(@PathVariable UUID categoryId) {
        ...
        return ResponseEntity.ok()
            .cacheControl(CacheControl.maxAge(Duration.ofMinutes(5)).cachePublic())
            .eTag("categoryId:" + category.getId() + ":v:" + category.getVersion())
